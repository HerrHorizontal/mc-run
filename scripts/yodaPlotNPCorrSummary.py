    ...
    for jet in jets.values():
        ...
        for i, (label, color, marker, lname, ao) in enumerate(
            ...
            if fits_given:
                ...
                for k, v in fits_given.items():
                    ...
                    if v in lname and jet["ident"] in k:
                        ...
                        if os.path.isfile(k):
                            with open(k, "r") as f:
                                fit_results = json.load(
                                    f, object_hook=json_numpy_obj_hook
                                )
                        else:
                            raise RuntimeError(
                                "Found match, but fit file {} doesn't exist!".format(k)
                            )
                        if match:
                            break
                if not match:
                    print(
                        "No matching fit file found for {}! Misconfiguration? Writing to {}".format(
                            lname, k
                        )
                    )
                    fit_results = fit(
            ...
            yEdgesFup = np.append(
                ...
                (fit_results["ys"] + fit_results["yerrs"])[-1],
            )
            yEdgesFdown = np.append(
                fit_results["ys"] - fit_results["yerrs"],
                (fit_results["ys"] - fit_results["yerrs"])[-1],
            )
            # axmain.plot(
