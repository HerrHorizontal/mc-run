...
namespace Rivet
{
  ...
  class ZplusJet_UE_ZFPT05ETA2 : public Analysis
  {
  public:
    DEFAULT_RIVET_ANALYSIS_CTOR(ZplusJet_UE_ZFPT05ETA2);
    ...
    void init()
    {
      ...
      // Charged final state particles cuts
    ...
    void analyze(const Event &event)
    {
      ...
      Particles particles =
          applyProjection<ChargedFinalState>(event, "cfs").particlesByPt(Cuts::pT > 0.5 * GeV && Cuts::abseta < etamax);
    ...
    const size_t _maxnleptons =
        numeric_limits<size_t>::max(); // maximium number of leptons
    ...
    // PARTICLE CUTS
    const double etamax = 2.0; // maximal pseudorap value taken into consideration.
    const double area = ((2. * etamax) * (2. * M_PI)) / 3.;
  };

  DECLARE_RIVET_PLUGIN(ZplusJet_UE_ZFPT05ETA2);
