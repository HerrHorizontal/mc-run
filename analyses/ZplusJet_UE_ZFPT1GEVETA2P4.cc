...
namespace Rivet
{
  ...
  class ZplusJet_UE_ZFPT1GEVETA2P4 : public Analysis
  {
  public:
    DEFAULT_RIVET_ANALYSIS_CTOR(ZplusJet_UE_ZFPT1GEVETA2P4);

    void init()
    {

      // Final state particles Cuts
      const FinalState fs(Cuts::abseta < 5.0 && Cuts::pT > 1.0 * GeV);
      declare(fs, "fs");
    ...
    void analyze(const Event &event)
    {
      ...
      Particles particles =
          applyProjection<ChargedFinalState>(event, "cfs").particlesByPt(Cuts::pT > 1.0 * GeV && Cuts::abseta < etamax);
  ...
  DECLARE_RIVET_PLUGIN(ZplusJet_UE_ZFPT1GEVETA2P4);
