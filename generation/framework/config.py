...
CAMPAIGN_MODS = {
    ...
    "sherpaLHC-LO-ZplusJet": dict(
        order="LO", label=r"Sherpa LO", lightencolor=0.5, linestyle="dashdot"
    ),
    "sherpaLHC-NLO-ZplusJet": dict(
        order="NLO", label=r"Sherpa NLO", lightencolor=0.7, linestyle="dashed"
    ),
    "sherpaLHC-ZJetMerging": dict(
        order="NLO", label=r"Sherpa Merged", lightencolor=1.0, linestyle="solid"
    ),
    ...
    "sherpaDijets_LO": dict(
        order="LO", label=r"Sherpa LO", lightencolor=0.5, linestyle="dashed"
    ),
    "sherpaDijets_NLO": dict(
        order="NLO", label=r"Sherpa NLO", lightencolor=0.3, linestyle="dashdot"
    ),
}
...
for region in ["Towards", "Transverse", "Away"]:
        ...
        "_".join((region, k)): dict(
            ...
            ident="_".join(
                [
                    ...
                    v["ident"][:2]
                    + "="
                    + "{:.6f}".format(float(v["ident"][2:10]) + 0.5),
                    v["ident"][10:12]
                    + "="
                    + "{:.6f}".format(float(v["ident"][12:]) + 0.5),
                ]
            ),
            label=v["label"],
        )
    ...
    BINS[f"YB0_zjet_RFUE_{region}"] = {
        k: v for k, v in BINS[f"zjet_RFUE_{region}"].items() if "YB_00_05" in k
    }
    BINS[f"YS0_zjet_RFUE_{region}"] = {
